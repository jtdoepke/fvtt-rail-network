...
// Will import from lightning-rail.mjs once implemented
// import { buildRouteSegments } from "./lightning-rail.mjs";

// Placeholder to make import work — replaced once lightning-rail.mjs exists
const mod = await import("./lightning-rail.mjs");
const { buildRouteSegments } = mod;

// Will be populated as functions are implemented
const getTrainPosition = mod.getTrainPosition ?? (() => { throw new Error("Not implemented"); });
const findAllActiveDepartures = mod.findAllActiveDepartures ?? (() => { throw new Error("Not implemented"); });
const resolveRoutePath = mod.resolveRoutePath ?? (() => { throw new Error("Not implemented"); });
const getActiveEvents = mod.getActiveEvents ?? (() => { throw new Error("Not implemented"); });
const computeEffectiveDelay = mod.computeEffectiveDelay ?? (() => { throw new Error("Not implemented"); });
const findExtraDepartures = mod.findExtraDepartures ?? (() => { throw new Error("Not implemented"); });
...
describe("resolveRoutePath", () => {
  // resolveRoutePath takes an array of segment configs (with inline paths)
  // and a worldTime, and returns a unified path array.
